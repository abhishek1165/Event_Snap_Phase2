...
const TARGETS = [
  { idx: 1,  conf: 98.7, label: "98.7%" },
  { idx: 6,  conf: 96.4, label: "96.4%" },
  { idx: 10, conf: 94.1, label: "94.1%" },
];
const TOTAL = 16;
...
export default function Viewfinder() {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      function loop() {
        ...
        tl = gsap.timeline({ onComplete: () => { timer = setTimeout(loop, 1400); } });
        tl.to(scanner, { opacity: 1, duration: .25 })
        ...
        TARGETS.forEach(({ idx, conf }, i) => {
          tl.call(() => {
            ...
            gsap.to(proxy, { conf, duration: .6, ease: "power2.out",
              onUpdate: () => setDisplay(d => ({ ...d, matched: m, conf: proxy.conf.toFixed(1) + "%" })) });
          }, null, `<${0.55 + i * 1.15}`);
        });
        tl.to(scanner, { opacity: 0, duration: .4 }, "+=0.7");
      }
      loop();
    };
    boot();
  ...
  const targetMap = Object.fromEntries(TARGETS.map(t => [t.idx, t.label]));

  return (
    <div style={{ background: T.paper, border: `1px solid ${T.frameSoft}`, borderRadius: 18, padding: 22, boxShadow: "0 30px 60px -30px rgba(0,0,0,.6)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <Eyebrow>Live match · Event Gallery</Eyebrow>
        <span style={{ fontFamily: MONO, fontSize: ".72rem", color: T.faint }}>16 FRAMES</span>
      </div>
      ...
      <div style={{ background: T.ink, border: `1px solid ${T.frameSoft}`, borderRadius: 10, padding: 14 }}>
        <div ref={gridRef} style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, height: 288, position: "relative" }}>
          {Array.from({ length: TOTAL }, (_, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{ position: "absolute", left: "50%", top: "38%", width: "46%", aspectRatio: 1, borderRadius: "50%", transform: "translate(-50%,-50%)", background: "radial-gradient(circle at 35% 30%,rgba(255,255,255,.16),rgba(255,255,255,.02) 60%),#2a2f3d" }} />
                {isMatch && (
                  <span style={{ position: "absolute", bottom: 4, left: 4, right: 4, fontFamily: MONO, fontSize: ".55rem", textAlign: "center", color: T.conf, background: "rgba(6,10,8,.7)", borderRadius: 4, padding: "2px 0" }}>
                    {targetMap[i]}
                  </span>
                )}
              </div>
          ...
          <div ref={scannerRef} style={{ position: "absolute", left: 0, right: 0, top: 0, height: 2, background: `linear-gradient(90deg,transparent,${T.signal2},${T.conf},${T.signal2},transparent)`, boxShadow: "0 0 16px 2px rgba(139,123,255,.55)" }} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 14 }}>
        {[["Scanned", display.scanned, false], ["Matched", display.matched, false], ["Confidence", display.conf, true]].map(([label, val, accent]) => (
          <div key={label} style={{ background: T.paper2, border: `1px solid ${T.frameSoft}`, borderRadius: 10, padding: "10px 12px" }}>
            <span style={{ display: "block", fontFamily: MONO, fontSize: ".62rem", letterSpacing: ".08em", color: T.faint, textTransform: "uppercase", marginBottom: 4 }}>{label}</span>
